...
func main() {
	...
	// Let's test for Silo ID 2 (which had manual bookings on 2026-05-29)
	siloID := int64(2)
	inventurDatum := "2026-05-30"
	inventurMenge := 5000.0

	fmt.Printf("--- Dry Run Futterinventur for Silo ID: %d, Date: %s, Qty: %.2f ---\n", siloID, inventurDatum, inventurMenge)
	...
	if maxDateStr <= inventurDatum {
		fmt.Printf("WARNING: maxDateStr (%s) <= inventurDatum (%s). Validation check would fail here.\n", maxDateStr, inventurDatum)
	}
	...
	// Commit (for testing purposes, we can commit and then delete them later, or we can rollback. Since it is dry-run, we will rollback)
	fmt.Println("Dry-run completed successfully without committing.")
}
