...
func main() {
	...
	// Query bookings list using MySQL repo
	queries := db.New(conn)
	res, err := queries.ListFutterBuchungen(ctx)
	if err != nil {
		log.Fatalf("Error fetching MySQL list: %v", err)
	}
	fmt.Printf("Loaded %d bookings successfully from MySQL.\n", len(res))

	// Clean up by deleting the test inserts
	_, err = conn.Exec("DELETE FROM FUTTER WHERE ID IN (?, ?)", idA, idB)
	if err != nil {
		log.Fatalf("Error cleaning up MySQL: %v", err)
	}
	fmt.Println("Cleaned up MySQL database inserts.")
}
