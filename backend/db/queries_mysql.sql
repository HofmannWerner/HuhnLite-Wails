...
-- name: CreateMwst :execresult
INSERT INTO MWST (MWSTKZ, PROZENT, KONTO)
VALUES (?, ?, ?) ;

-- name: UpdateMwst :execresult
UPDATE MWST
...
-- name: CreateRasse :execresult
INSERT INTO RASSE (RASSE)
VALUES (?) ;

-- name: UpdateRasse :execresult
UPDATE RASSE
SET RASSE = ?
WHERE ID = ? ;
...
-- name: CreateStall :execresult
INSERT INTO STALL (STALLNUMMER, BEZEICHNUNG) 
VALUES (?, ?) ;

-- name: UpdateStall :execresult
UPDATE STALL 
...
-- name: CreateHerdeChecked :execresult
INSERT INTO HERDEN (
...
-- name: CreateFirmenparameter :execresult
...
-- name: UpdateFirmenparameter :execresult
...
WHERE (sqlc.arg(id_herden) = -1 OR BUCHUNG.ID_HERDEN = sqlc.arg(id_herden))
  ...
  AND (sqlc.arg(year) = '' OR YEAR(BUCHUNGSDATUM) = sqlc.arg(year))
  AND (sqlc.arg(quarter) = 0 OR (CAST(MONTH(BUCHUNGSDATUM) as signed) + 2) / 3 = sqlc.arg(quarter))
  AND (sqlc.arg(month) = 0 OR CAST(MONTH(BUCHUNGSDATUM) as signed) = sqlc.arg(month))
  AND VERMITTELT IN ('N', 'V');
...
-- name: CreateFutterBuchung :execresult
...
-- name: UpdateFutterBuchung :execresult
UPDATE FUTTER
SET ID_SILO     = ?,
    SILONUMMER  = ?,
    HERDENR     = ?,
...
-- name: CreateTierbewegung :execresult
...
-- name: UpdateTierbewegung :execresult
...
-- name: UpdateKostentabkopf :execresult
...
-- name: CreateUebersetzung :execresult
INSERT INTO UEBERSETZUNGEN (ID_TEXTE, SPRACHE_KZ, BETREFF, INHALT)
VALUES (?, ?, ?, ?) ;

-- name: UpdateUebersetzung :execresult
...
-- name: UpsertUebersetzung :execresult
INSERT INTO UEBERSETZUNGEN (ID_TEXTE, SPRACHE_KZ, BETREFF, INHALT)
...
-- name: UpdateTabellenkopf :execresult
UPDATE TABELLENKOPF
SET TABELLENNUMMER = ?,
    BEZEICHNUNG    = ?,
    ANLAGEDATUM    = ?,
...
-- name: CreateTabellenkopf :execresult
...
-- name: CreateLSLKlassik :execresult
...
-- name: UpdateLSLKlassik :execresult
UPDATE LSLKLASSIK
SET TABELLENNUMMER = ?,
    ALTERINWOCHEN  = ?,
    EIZAHLKUM      = ?,
...
-- name: AddEilagerBuchung :execresult
...
-- name: UpdateEilagerBuchung :execresult
...
-- name: CreateLagerplatz :execresult
INSERT INTO LAGERPLATZ (ID_EILAGER, BEZEICHNUNG, BEMERKUNG)
VALUES (?, ?, ?) ;

-- name: UpdateLagerplatz :execresult
UPDATE LAGERPLATZ
...
-- name: CreateDynamischeSQL :execresult
...
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ;
-- name: UpdateDynamischeSQL :execresult
...
-- name: UpsertFieldTranslation :execresult
INSERT INTO TRANSLATEFELDNAMEN (ID_FELD_KATALOG, SPRACHE_KZ, BETREFF, INHALT)
VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE BETREFF = VALUES(BETREFF),
    INHALT = VALUES(INHALT)
    ;

-- name: CreateFieldKatalog :execresult
...
-- name: CreateBenutzerProfil :execresult
...
-- name: UpdateBenutzerProfil :execresult
...
-- name: CreateBenutzer :execresult
...
-- name: CreateShowTV :execresult
INSERT INTO SHOWTV (TVNAME, SHOWIT)
VALUES (?, ?) ;

-- name: UpdateShowTV :execresult
UPDATE SHOWTV
SET TVNAME = ?,
    SHOWIT = ?
WHERE ID = ? ;
...
-- name: CreateFuttersorte :execresult
INSERT INTO FUTTERSORTEN (BEZEICHNUNG)
VALUES (?) ;

-- name: UpdateFuttersorte :execresult
UPDATE FUTTERSORTEN
SET BEZEICHNUNG = ?
WHERE ID = ? ;
